...
class PendulumConfig(RobotConfig):
    def __init__(self,args):
        ...
        self.sensor_dimensions = {"acc": 3, "gyr": 3, "orient": 4, "motor_torque" : 1, "motor_torque_abs" : 1, "poti": 1, "poti_d": 1, "poti_integral":1, "current":1}
        ...
        self.set_sensors(['poti_integral'])
    ...
    def set_sensors(self, use_sensors):
        ...
        self.numsen = np.sum([self.sensor_dimensions[sensor] for sensor in self.use_sensors]) * self.smoothing_length
        print self.numsen
    ...
    def get_input(self):
        ...
        if self.smp_control.cnt_main < 5: return

        if 'acc' in self.use_sensors:
            imu_vec_acc = (self.imu_msg.linear_acceleration.x, self.imu_msg.linear_acceleration.y, self.imu_msg.linear_acceleration.z)
            sensor_tupel += (imu_vec_acc)

        if 'gyr' in self.use_sensors:
            imu_vec_gyr = (self.imu_msg.angular_velocity.x, self.imu_msg.angular_velocity.y, self.imu_msg.angular_velocity.z)
            sensor_tupel += (imu_vec_gyr)

        if 'orient' in self.use_sensors:
            imu_vec_orient = (self.imu_msg.orientation.x, self.imu_msg.orientation.y, self.imu_msg.orientation.z, self.imu_msg.orientation.w)
        ...
        for i in range(self.smoothing_length):
            ...
            if self.smp_control.cnt_main == 5:
                ...
                self.sensor_vec[s:e] = old_sensor_vec[s:e] * self.smoothings[i] + new_sensor_vec * (1.-self.smoothings[i])
