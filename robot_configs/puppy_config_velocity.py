...
class PuppyConfigVelocity():
    ...
    def send_output(self, algorithm_output):
        # velocity control
        self.motor_commands = (self.motor_commands + algorithm_output).clip(-1,1)
        self.motor_position_estimate = self.motor_position_estimate * 0.3 + self.motor_commands * 0.7

        # write the commands to the message and publish them
        self.msg_motors.data = self.motor_commands * self.output_gain
